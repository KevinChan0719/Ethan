...
let main = new Vue({
  ...
  data () {
    return {
      press: this.$t("press.main")
    }
  },
  computed: {
    main () {
      return this.$t("press.main")
    }
  },
  created() {
    ...
    this.$t("press.main").map((e, i) => {
      this.$set(this.press[i], "title", e.title);
      this.$set(this.press[i], "general", e.general);
    });
  },
  methods: {
    data(day, month) {
    ...
    changeLang(lang) {
      this.$refs.changeLang.style.display = "none";
      if (lang === "CN") {
        document.title = "比特币支付-媒体报道";
        this.$set(this.$i18n, "locale", "zh_CN");
      } else {
        document.title = "BitcoinPay-Press";
        this.$set(this.$i18n, "locale", "en_US");
      }
      console.log(this.main)
      this.press = this.main
    }
  }
});
